const PromoSection = ({ products }) => {
  // Get all tags and count how many times they appear
  ...
  // Sort by count and get the top 3 tags
  tagCounts.sort((a, b) => b.count - a.count);
  const popularTags = tagCounts.slice(0, 3).map((tag) => tag.name);

  // Filter products by popular tags and get matching image
  ...
  const popularProductsWithImages = popularProducts.map((product) => {
    const matchingImage = products.find(
      (p) => p.id !== product.id && p.tags.some((tag) => product.tags.includes(tag))
    );
    return {
